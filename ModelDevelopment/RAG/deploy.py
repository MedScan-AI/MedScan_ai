...
class RAGModelDeployer:
    ...
    def _create_dummy_saved_model(self, gcs_dir: str):
        """Create a minimal dummy saved_model.pb for Vertex AI registration validation"""
        try:
            # Create a minimal valid TensorFlow SavedModel protobuf
            # This is only for Vertex AI registration validation - not used for actual inference
            # Our RAG inference service loads artifacts directly from GCS, not from this SavedModel
            
            # Minimal SavedModel protobuf structure
            # This creates a valid but empty SavedModel that passes Vertex AI validation
            import struct
            
            # Create a minimal valid SavedModel.pb
            # Format: protobuf with SavedModel message
            # This is a simplified version that just satisfies the file existence check
            dummy_pb = (
                b'\x0a\x0csaved_model\x12\x0b1.0.0-rc2'  # Basic SavedModel structure
            )
            
            # Upload to GCS in the same directory as other artifacts
            blob_path = f"{gcs_dir}/saved_model.pb"
            blob = self.bucket.blob(blob_path)
            blob.upload_from_string(dummy_pb, content_type='application/octet-stream')
            logger.info(f"Created dummy saved_model.pb at gs://{self.bucket_name}/{blob_path} for Vertex AI registration")
            
            # Verify it was uploaded
            if blob.exists():
                logger.info(f"Verified: saved_model.pb exists at gs://{self.bucket_name}/{blob_path}")
            else:
                logger.error(f"ERROR: saved_model.pb was not uploaded successfully to {blob_path}")
        except Exception as e:
            logger.error(f"Failed to create dummy saved_model.pb: {e}")
            import traceback
            logger.error(traceback.format_exc())
            raise  # Re-raise to fail the deployment if this critical step fails
    ...
    def upload_model_artifacts(
        ...
        # Create dummy saved_model.pb for Vertex AI registration validation
        # This is required by TensorFlow container but won't be used for actual inference
        self._create_dummy_saved_model("RAG/models/latest")
    ...
    def register_rag_model(
        ...
        # For RAG models, we use a generic container for registration.
        # Actual inference happens in our custom inference service, not via Vertex AI endpoints.
        # The model is registered in Vertex AI Model Registry for tracking purposes only.
        model = aiplatform.Model.upload(
            display_name=display_name,
            artifact_uri=artifact_uri,
            # Use a standard container - required by Vertex AI but won't be used for serving
            # Our RAG inference service loads artifacts directly from GCS
            # Using TensorFlow container as it's more lenient with artifact formats
    ...
    def deploy_rag_model(
        ...
        # Register model - use directory URI (parent directory of config)
        # Vertex AI expects a directory, not a file
        config_uri = uris['config']
        if config_uri.startswith('gs://'):
            # Extract directory path (remove filename)
            parts = config_uri.replace('gs://', '').split('/', 1)
            artifact_dir = f"gs://{parts[0]}/{'/'.join(parts[1].split('/')[:-1])}/"
        else:
            artifact_dir = str(Path(config_uri).parent) + '/'
        
        model = self.register_rag_model(
            display_name="medscan-rag-model",
            artifact_uri=artifact_dir,  # Use directory URI
            metadata=metadata
        )

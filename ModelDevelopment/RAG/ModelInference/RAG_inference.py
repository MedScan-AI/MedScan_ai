...
class RAGDataLoader:
    ...
    def _get_gcs_path(self, data_type: str) -> str:
        ...
        paths = {
            'embeddings': 'RAG/index/embeddings_latest.json',  # Updated to match upload
            'index': 'RAG/index/index_latest.bin',  # Updated to match upload
            'data': 'RAG/index/data_latest.pkl'  # Updated to match upload
        }
    ...
    def _get_local_fallback(self, data_type: str) -> Path:
        ...
        files = {
            'embeddings': ['embeddings_latest.json', 'embeddings.json'],  # Try _latest first
            'index': ['index_latest.bin', 'index.bin'],  # Try _latest first
            'data': ['data_latest.pkl', 'data.pkl']  # Try _latest first
        }
        
        filenames = files.get(data_type, [''])

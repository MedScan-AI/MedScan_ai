"""
Training script for Vision Transformer (ViT) model.
Trains ViT for Tuberculosis (TB) and Lung Cancer detection.
...
# ViT doesn't use pre-trained models from applications
...
class TileClassTokenLayer(Layer):
    """Custom layer to tile class token across batch."""
    def call(self, inputs):
        class_token, patches = inputs
        # Get batch size from patches tensor dynamically
        batch_size = tf.shape(patches)[0]
        # Get projection_dim from class_token
        projection_dim = tf.shape(class_token)[-1]
        # Extract a single token using gather (dynamic indexing)
        # class_token shape is [batch_size, 1, projection_dim] from embedding
        # We want to get [1, 1, projection_dim] and tile it
        single_token = tf.gather(class_token, [0], axis=0)  # Shape: [1, 1, projection_dim]
        # Ensure we have the right shape before tiling
        single_token = tf.reshape(single_token, [1, 1, projection_dim])
        # Create tiling pattern: [batch_size, 1, 1]
        tiling = tf.stack([batch_size, 1, 1])
        # Tile to match batch size: [batch_size, 1, projection_dim]
        return tf.tile(single_token, tiling)


class ExtractClassTokenLayer(Layer):
    """Custom layer to extract class token (first token)."""
    def call(self, inputs):
        return inputs[:, 0]


class ModelBuilder:
    """Build Vision Transformer (ViT) model architecture."""
    ...
    def build_vit(
        ...
        """
        Build Vision Transformer (ViT) model using ViT-Small/16 configuration.
        ViT-Small has ~22M parameters, similar to ResNet50's ~25M parameters.
        ...
        """
        vit_config = self.config.get('models.vit', {})
        patch_size = vit_config.get('patch_size', 16)
        projection_dim = vit_config.get('projection_dim', 384)
        num_heads = vit_config.get('num_heads', 6)
        transformer_layers = vit_config.get('transformer_layers', 12)
        mlp_ratio = vit_config.get('mlp_ratio', 4)
        attention_dropout = vit_config.get('attention_dropout', 0.1)
        mlp_dropout = vit_config.get('mlp_dropout', 0.1)
        mlp_head_units = vit_config.get('mlp_head_units', [1024, 512])
        mlp_head_dropout = vit_config.get('mlp_head_dropout', 0.5)
        ...
        def create_vit_classifier():
            ...
            # Reshape patches: (batch, height_patches, width_patches, dim) -> (batch, num_patches, dim)
            patches_reshaped = layers.Reshape((num_patches, projection_dim))(patches)
            ...
            # Class token (learnable) - use Embedding layer for a single learnable token
            class_token_embedding = layers.Embedding(
                input_dim=1,
                output_dim=projection_dim,
                name="class_token_embedding"
            )
            # Create a constant index [0] for the class token using custom layer
            class_token_idx = ClassTokenLayer(name="class_token_idx")(inputs)
            class_token_single = class_token_embedding(class_token_idx)
            # class_token_single shape: [batch_size, 1, projection_dim] - already correct!
            # No need to tile, the embedding already produces the right batch dimension
            class_token = class_token_single
            
            # Concatenate class token with patches
            encoded = layers.Concatenate(axis=1)([class_token, patches_reshaped])
            ...
            for i in range(transformer_layers):
                ...
                attention_output = layers.MultiHeadAttention(
                    ...
                    name=f"transformer_layer_{i}_attention"
                )(encoded, encoded)
                
                # Add & Norm (first)
                x = layers.Add(name=f"transformer_layer_{i}_add1")([attention_output, encoded])
                x = layers.LayerNormalization(name=f"transformer_layer_{i}_norm1")(x)
                
                # MLP (ViT-Small uses 4x hidden size = 1536)
                mlp = layers.Dense(projection_dim * mlp_ratio, activation='gelu', name=f"transformer_layer_{i}_mlp1")(x)
                mlp = layers.Dropout(mlp_dropout, name=f"transformer_layer_{i}_dropout1")(mlp)
                mlp = layers.Dense(projection_dim, name=f"transformer_layer_{i}_mlp2")(mlp)
                mlp = layers.Dropout(mlp_dropout, name=f"transformer_layer_{i}_dropout2")(mlp)
                
                # Add & Norm (second)
                encoded = layers.Add(name=f"transformer_layer_{i}_add2")([x, mlp])
                encoded = layers.LayerNormalization(name=f"transformer_layer_{i}_norm2")(encoded)
            
            # Extract class token (first token) using custom layer
            representation = ExtractClassTokenLayer(name="extract_class_token")(encoded)
            
            # MLP Head
            for i, units in enumerate(mlp_head_units):
                representation = layers.Dense(units, activation='gelu', name=f"mlp_head_{i}")(representation)
                representation = layers.Dropout(mlp_head_dropout, name=f"mlp_head_dropout_{i}")(representation)
        ...
        learning_rate = vit_config.get('learning_rate', 0.001)
        model.compile(
...
class HyperparameterTuner:
    ...
    def _build_tunable_vit(self, hp, input_shape: Tuple[int, int, int], num_classes: int) -> models.Model:
        """Build tunable ViT model."""
        search_space = self.tuning_config.get('search_space', {})
        vit_space = search_space.get('vit', {})
        ...
        projection_dim = hp.Choice(
            'projection_dim',
            values=vit_space.get('projection_dim', {}).get('values', [256, 384, 512])
        )
        
        num_heads = hp.Choice(
            'num_heads',
            values=vit_space.get('num_heads', {}).get('values', [4, 6, 8])
        )
        
        transformer_layers = hp.Choice(
            'transformer_layers',
            values=vit_space.get('transformer_layers', {}).get('values', [8, 12, 16])
        )
        
        mlp_head_units_1 = hp.Choice(
            'mlp_head_units_1',
            values=vit_space.get('mlp_head_units_1', {}).get('values', [512, 1024, 2048])
        )
        
        mlp_head_units_2 = hp.Choice(
            'mlp_head_units_2',
            values=vit_space.get('mlp_head_units_2', {}).get('values', [256, 512, 1024])
        )
        ...
        # Create class token using Keras layers - use Embedding layer for a single learnable token
        class_token_embedding = layers.Embedding(
            input_dim=1,
            output_dim=projection_dim,
            name="class_token_embedding"
        ...
        for i in range(transformer_layers):
            ...
            mlp = layers.Dense(projection_dim * 4, activation='gelu', name=f"transformer_layer_{i}_mlp1")(x)
            mlp = layers.Dropout(0.1, name=f"transformer_layer_{i}_dropout1")(mlp)
            mlp = layers.Dense(projection_dim, name=f"transformer_layer_{i}_mlp2")(mlp)
            mlp = layers.Dropout(0.1, name=f"transformer_layer_{i}_dropout2")(mlp)
            
            encoded = layers.Add(name=f"transformer_layer_{i}_add2")([x, mlp])
            encoded = layers.LayerNormalization(name=f"transformer_layer_{i}_norm2")(encoded)
        
        representation = ExtractClassTokenLayer(name="extract_class_token")(encoded)
        representation = layers.Dense(mlp_head_units_1, activation='gelu', name="mlp_head_0")(representation)
        representation = layers.Dropout(0.5, name="mlp_head_dropout_0")(representation)
        representation = layers.Dense(mlp_head_units_2, activation='gelu', name="mlp_head_1")(representation)
        representation = layers.Dropout(0.5, name="mlp_head_dropout_1")(representation)
        outputs = layers.Dense(num_classes, activation='softmax', name="classification_head")(representation)
    ...
    def tune_model(
        ...
        """
        Tune hyperparameters for ViT model.
        ...
        # Create tuner
        model_builder_fn = lambda hp: self._build_tunable_vit(hp, input_shape, num_classes)
...
def train_dataset(
    ...
    """
    Train ViT model for a specific dataset.
    ...
    logger.info(f"Starting ViT training for {dataset_name} dataset")
    ...
    # Initialize MLflow
    mlflow.set_experiment(f"{dataset_name}_vit_training")
    ...
    if tuning_enabled:
        logger.info("Hyperparameter tuning is enabled. Tuning ViT...")
        ...
        model = model_builder.build_vit(input_shape, num_classes)
        tuned = False
    ...
    with mlflow.start_run(run_name=f"{dataset_name}_{model_name}"):
        # Enable unsafe deserialization for Keras to handle custom layers
        # This is safe since we control the model architecture
        ...
        # For ViT models with custom layers, create signature manually to avoid deserialization issues
        # Create signature from model input/output shapes instead of using input_example
        logger.info("Logging model to MLflow with manual signature...")
        from mlflow.models import ModelSignature
        from mlflow.types import Schema, TensorSpec
        
        # Create input schema from model input shape
        input_schema = Schema([TensorSpec(np.dtype(np.float32), (-1, *input_shape), "image")])
        # Create output schema from model output shape
        output_schema = Schema([TensorSpec(np.dtype(np.float32), (-1, num_classes), "predictions")])
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"ViT training completed for {dataset_name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train ViT model for medical image classification')
    parser.add_argument(
        '--config',
        type=str,
        default=None,
    ...
    logger.info("ViT training completed!")
